...
| **pygo (8 hub)** | **3496** | **11 ms** | 271 ms | 526 ms | 29 |
...
a 100 ms CPU block waits). pygo holds **p50 = 11 ms (37× better) and 7–10×
the throughput** — its 8 hubs absorb the CPU blocks on other cores.

## Single core, GIL'd 3.13 (incl. gevent; pygo = 1 hub)
...
| pygo (1 hub) | 708 | 281 ms | 677 ms | 1047 |

**On one core, everyone collapses** — the pathological tier is CPU-bound, and
no scheduler can make one core do more work. pygo-1hub ≈ asyncio (preemption
buys a little fairness, not throughput). 

## Conclusion (honest)

pygo's advantage on the pathological tier is **multi-core (free-threading)**,
