#!/usr/bin/env sh
# On-CPU flame graph of a pygo workload via perf timer-sampling.
#
...
# -O2 -g, so `--call-graph dwarf` resolves pygo_core frames despite -O2
...
FREQ="${FREQ:-1999}"
DATA="$(mktemp /tmp/pygo_perf.XXXXXX).data"
mkdir -p "$(dirname "$OUT")"
