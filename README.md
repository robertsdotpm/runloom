...
| **C**  | Free-threaded Python 3.13t + **M:N work-stealing scheduler** + yield-in-hub + netpoll-in-hub + **sleep-in-hub** | **done** — pygo runs on 3.13t with GIL disabled; Chase-Lev work-stealing deques per hub (with per-hub MPSC submission list so external producers don't race the deque owner); thread-local current-hub pointer routes `sched_yield` / `wait_fd` / `sched_sleep` from a goroutine on hub H back to H's local FIFO / sleep heap; shared epoll fd with locked parked list, any hub drives `epoll_wait` when idle; per-hub sleep heap with idle-wait capped by next timer; 2.5× parallel speedup on 8 cores, M:N + I/O echo + sleep benches all clean across hubs |
...
Real-hardware ARM validation (Apple Silicon / Linux ARM) and further
perf polish on the slow-path snap.
