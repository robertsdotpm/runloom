...
- **`go(callable, *args, stack_size=0, **kwargs)`** — spawn a goroutine. The
  dispatch is the interesting part: it reads `runloom_c.mn_hub_count()` (not a mode
  flag) so the *same* `go()` works from anywhere — inside a hub goroutine or from
  ...
  - `stack_size=` is *runloom's* keyword (the per-goroutine C stack), popped before
...
- **`run(n, main_fn=None)`** — THE entry point. `n` is required and explicit
  because M:N is a *different correctness model* (goroutines run Python in
...
- **`sleep(seconds)`** — cooperative; falls back to `time.sleep` outside a
  goroutine (so the same name works in either context).
...
- **`blocking(fn, …)`** — offload a blocking/CPU call off the hub (spec 08); runs
  inline when not on a goroutine.
...
(it only grows at yields), so a small-stack goroutine hitting it cold would
...
the path a goroutine later takes is shallow. This is *why* small default
goroutine stacks are safe in practice (spec 10).
...
`NewTicker`/`Stop/Reset`, `Sleep`. **All driven by a backing goroutine that sleeps
...
`Stop`/`Reset` clean — the sleeping `fire()` goroutine checks `self._gen == gen` on
...
helper goroutine must check the hub count, or it hangs under `mn_run`).
...
goroutine that `sched_sleep`s to the deadline then cancels (inheriting a tighter
...
`sync` / `aio` / `monkey` are *different front-ends to the same goroutines* and can
...
3. **`prewarm_stdlib()` runs on the main thread before any goroutine** — the
   enabler for small stacks (the deep `getaddrinfo` import burst).
4. **Any helper-goroutine spawn checks the hub count** (`time`, `context`) or it
   hangs under `mn_run`.
