# 03 — The CPython per-goroutine state snapshot
...
(one per OS thread), not on the C stack — and runloom multiplexes many goroutines
onto one tstate (one per hub). If you swap C stacks without saving/restoring those
tstate fields, goroutine B resumes reading goroutine A's interpreter state. The
...
> prototype could run one goroutine but tangled the `tstate.cframe` chain across
...
Per goroutine, keep a `runloom_pystate_snap`. On **yield/park/sleep**, *copy out*
...
- **contextvars `context`** (owned ref) — so each goroutine has its own
...
- **datastack arena pointers** (`datastack_chunk`, `datastack_top`,
  ...
  arena, not the C stack. Each goroutine needs its own frame chain; these three
  pointers are the frame arena's cursor and must move with the goroutine.
...
- **`current_exception`** (`tstate->current_exception`) — the in-flight *unraised*
  ...
  goroutine yields with this non-NULL, another overwrites it, and the first reads
...
Because frames live in the datastack arena, a first-run goroutine needs a root
...
this matches greenlet's `did_finish`. A parked goroutine's *idle* datastack tail
...
goroutine parks mid-comparison), the held mutex would stay locked across the swap
— and because runloom does **not** detach the tstate on a cooperative park, every
*other* hub then deadlocks on that mutex, and the chain leaks across goroutines.
...
- **Per-hub tstate (default).** One `PyThreadState` per hub; goroutines share it
  via this snap/restore dance. A started goroutine's suspended eval frame caches
  its origin hub's tstate, so it is **not** safely migratable to another hub —
  which is *why work-stealing only steals fresh goroutines* (spec 05). `snap`/`load`
  do the real work.
...
2. **Save `current_exception` and the recursion counters.** Omitting either is a
   real crash (stale-exception segfault; cross-goroutine recursion-limit leak).
...
5. **Do not migrate a started goroutine across hubs in per-hub-tstate mode.** Its
