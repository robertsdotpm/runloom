...
Small per-fiber stacks are what make a million fibers affordable (spec 01
...
A fiber running low on stack is defended the way the main thread is, scaled
down:

1. **Deep recursion raises `RecursionError`, not a crash.** At fiber entry
   the per-tstate recursion counters are *reset for this fiber* —
   ...
   counter is "NOT lowered per fiber" — that doc is stale; the code lowers it
   to 200 at entry.
2. **Stacks grow on demand** (copy-grow / Path A, spec 01). A fiber that
...
4. **The crash reporter classifies the guard fault** (spec 11) into "fiber N
...
5. **CPython's stack-hungry error paths are neutralized.** A missing-attribute
   ...
   fiber* (off-fiber the stock hint is byte-for-byte preserved), so
   ...
   ~32 KB is the *hint buffer*, not the fiber stack default — see the next
   section; do not confuse them.)

The **residual**: a *single native/FFI C frame larger than the whole fiber
stack* — a non-probing extension that jumps the guard in one allocation. That's
the one case the layers can't catch; the fix is to size that fiber up front
...
two single frames**, both handled by *removing the frame from the fiber path*
...
- **`select.select` — 50.9 KB** (three `pylist[FD_SETSIZE+1]` arrays). Reimplemented
  ...
  `wait_fd`) so CPython's `select_select_impl` is never reached from a fiber
  (spec 06/14).
- **first `ssl` use — ~40 KB** (OpenSSL's one-time library init). Warmed on the
  ...
  main stack, pre-paying it off any fiber.
...
JSON in a fiber) is **safe**. `eval`/`exec` of a string compile internally in
C and need the caller's namespace, so they're the documented residual (use
`offload()` or a roomier stack).

> **The one-line rule for writing stack-predictable fibers**: keep stack depth
...
1. **Calibration** ([runloom_sched_core.c.inc:17-47](../src/runloom_c/runloom_sched_core.c.inc#L17)).
   The first 1000 fibers run on the 512 KB default; their HWMs are scanned
...
costs no RAM until touched. On fiber completion the stack returns to a
...
idle pages. Net: 10k idle fibers cost ~their actual usage, not their
...
4. **The two fat stdlib frames are removed from the fiber path** (cooperative
