...
# auto-offload only kicks in above a size threshold (PYGO_OFFLOAD_BYTES, default
...
# have to -- it just happens.  pygo.monkey.offload() remains for everything else
...
def _heavy_threshold():
    try:
        return int(os.environ.get("PYGO_OFFLOAD_BYTES", 256 * 1024))
...
def _make_heavy(orig, data_index, data_kwnames, always):
    ...
    wrapper.__pygo_heavy__ = True       # marker for tests / introspection
    return wrapper
