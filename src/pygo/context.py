"""pygo.context -- Go-style context.Context for cancellation.
...
Patterns we support, lifted straight from Go:
    ...
    ctx, cancel = pygo.context.WithTimeout(parent, 5.0)
    ctx, cancel = pygo.context.WithDeadline(parent, monotonic + 5.0)
...
# Error sentinels (strings -- pygo deliberately avoids custom exception
...
def WithDeadline(parent, deadline_monotonic):
    ...
    def deadline_waker():
        ...
        if remaining > 0:
            pygo_core.sched_sleep(remaining)
        if ctx._err is None:
            ctx._cancel(DEADLINE_EXCEEDED)

    pygo_core.go(deadline_waker)
    return ctx, cancel
