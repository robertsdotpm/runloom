/* netpoll_iocp.h -- Windows IOCP+AFD backend for runloom netpoll.
 *
 ...
 * three backends share the same runloom_netpoll_wait_fd / _pump
 * contract.
 */
#ifndef RUNLOOM_NETPOLL_IOCP_H
#define RUNLOOM_NETPOLL_IOCP_H
...
/* Tear down the IOCP + AFD handles.  Called from runloom_netpoll_fini. */
 ...
 * runloom_iocp_wake_armed() reports whether the IOCP exists so a wake can
 * be posted; runloom_iocp_wake() posts it.  Both are no-ops/-1 when IOCP
 * was never created. */
int runloom_iocp_wake_armed(void);
int runloom_iocp_wake(void);

#endif /* RUNLOOM_OS_WINDOWS */
#endif /* RUNLOOM_NETPOLL_IOCP_H */
