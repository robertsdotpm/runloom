/* plat_compat.h -- runtime shims that let the rest of runloom_c use one
 ...
 *   3. clock & sleep  runloom_monotonic_ns / runloom_sleep_ns
 *
...
/* ============================================================ */
#if defined(RUNLOOM_OS_WINDOWS)
typedef CRITICAL_SECTION runloom_mutex_t;
...
/* ============================================================ */
#if defined(RUNLOOM_OS_WINDOWS)
typedef HANDLE runloom_thread_t;
typedef unsigned (__stdcall *runloom_thread_fn)(void *);
...
#  define RUNLOOM_THREAD_RET     unsigned __stdcall
#  define RUNLOOM_THREAD_RETURN(v)  return (unsigned)(uintptr_t)(v)
#else
 ...
 * Python, which is far newer than XP.  Like runloom_mutex_t there is no
 * static initialiser on Windows, so callers must runloom_cond_init() once. */
#if defined(RUNLOOM_OS_WINDOWS)
typedef CONDITION_VARIABLE runloom_cond_t;
RUNLOOM_INLINE int  runloom_cond_init(runloom_cond_t *c) {
    InitializeConditionVariable(c); return 0;
}
RUNLOOM_INLINE void runloom_cond_destroy(runloom_cond_t *c) { (void)c; }
RUNLOOM_INLINE void runloom_cond_wait(runloom_cond_t *c, runloom_mutex_t *m) {
    SleepConditionVariableCS(c, m, INFINITE);
}
RUNLOOM_INLINE void runloom_cond_signal(runloom_cond_t *c)    { WakeConditionVariable(c); }
RUNLOOM_INLINE void runloom_cond_broadcast(runloom_cond_t *c) { WakeAllConditionVariable(c); }
#else
...
/* ============================================================ */
#if defined(RUNLOOM_OS_WINDOWS)
RUNLOOM_INLINE void runloom_winsock_init(void) {
 ...
 * call_soon_threadsafe) never touch netpoll.  Opt out with RUNLOOM_WIN_TIMER_RES=0.
...
#else
RUNLOOM_INLINE void runloom_winsock_init(void) { /* no-op on POSIX */ }
RUNLOOM_INLINE void runloom_timer_res_init(void) { /* no-op on POSIX */ }
#endif
