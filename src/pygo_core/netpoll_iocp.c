...
static HANDLE pygo_afd_handle = INVALID_HANDLE_VALUE;
static HANDLE pygo_iocp_handle = NULL;
static volatile LONG pygo_iocp_inited = 0;

/* Per-poll context block.  Held alive by the IOCP itself until
 * GetQueuedCompletionStatus returns it; freed by the caller after
 * pygo_iocp_wait consumes the completion. */
typedef struct pygo_poll_ctx {
    ...
    int             fd;             /* the socket */
    int             requested;      /* PYGO_NETPOLL_* mask */
} pygo_poll_ctx_t;

/* Translate PYGO_NETPOLL_READ/WRITE to AFD event mask. */
static ULONG pygo_to_afd_events(int events)
{
    ULONG mask = 0;
...
int pygo_iocp_init(void)
{
    ...
    static const WCHAR afd_path[] = L"\\Device\\Afd\\Pygo";

    if (InterlockedCompareExchange(&pygo_iocp_inited, 1, 0) != 0) {
        /* Someone else won the init race; spin until they finish. */
        while (pygo_iocp_inited != 2) { /* spin */ }
        return (pygo_afd_handle != INVALID_HANDLE_VALUE) ? 0 : -1;
    }
    ...
    st = pf_NtCreateFile(
        ...
        NULL, 0);
    if (!NT_SUCCESS(st)) {
        pygo_afd_handle = INVALID_HANDLE_VALUE;
        goto fail;
    }

    /* Create the IOCP and associate AFD with it. */
    pygo_iocp_handle = CreateIoCompletionPort(
        INVALID_HANDLE_VALUE, NULL, 0, 0);
    if (pygo_iocp_handle == NULL) goto fail;
    if (CreateIoCompletionPort(pygo_afd_handle, pygo_iocp_handle,
                               (ULONG_PTR)0, 0) == NULL) {
        goto fail;
    }
    ...
    SetFileCompletionNotificationModes(pygo_afd_handle,
...
fail:
    if (pygo_iocp_handle != NULL) {
        CloseHandle(pygo_iocp_handle);
        pygo_iocp_handle = NULL;
    }
    if (pygo_afd_handle != INVALID_HANDLE_VALUE) {
        CloseHandle(pygo_afd_handle);
        pygo_afd_handle = INVALID_HANDLE_VALUE;
    }
    pygo_iocp_inited = 2;       /* mark "init attempted, failed" */
    return -1;
}
...
int pygo_iocp_submit(int fd, int events, long long timeout_ns)
{
    ...
    if (pygo_iocp_inited != 2 || pygo_afd_handle == INVALID_HANDLE_VALUE) {
        return -1;
    }

    base = pygo_iocp_base_socket((SOCKET)(uintptr_t)fd);
    if (base == INVALID_SOCKET) return -1;

    ctx = (pygo_poll_ctx_t *)calloc(1, sizeof(*ctx));
    ...
    ctx->poll_info.Handles[0].Events = pygo_to_afd_events(events);
                   ...
                   int *out_fd, int *out_events)
{
    ...
    ok = GetQueuedCompletionStatus(pygo_iocp_handle, &bytes, &key, &ov, ms);
    if (!ok && ov == NULL) {
        /* timeout */
        return 0;
    }
