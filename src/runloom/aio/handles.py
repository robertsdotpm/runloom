...
# We list BOTH; RunloomTask (our own) is excluded at the call site since it wants
...
def _run_stock_task_cb(loop, cb, fut):
    ...
    # slot is empty when a call_soon callback runs.  runloom CANNOT: RunloomTask._driver
    ...
    #
    # The parked RunloomTask is suspended, not actually executing, so clearing its
...
# Every RunloomTask, across ALL loops on this process.  The runloom scheduler is one
# per OS thread (shared by every RunloomEventLoop on that thread), so loop.close()
...
# ====================================================================
# RunloomFuture -- pure-Python Future replacement with synchronous-fire
