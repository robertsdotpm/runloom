...
The wins come from amortizing RunloomTask setup cost across many awaits.
The losses come from RunloomTask creation + Chan alloc being heavier
...
Compatibility:
  ...
  * loop.add_reader / add_writer: work (level-triggered like asyncio's
    default selector loop, just driven by runloom's netpoll).
  * asyncio.start_server / open_connection (Transport+Protocol stack):
    NOT in this MVP -- for I/O, prefer `runloom.monkey.patch()` and write
...
Use:
    ...
    asyncio.run(main())                  # routed through runloom
...
# Public API surface (kept importable as runloom.aio.<name>).
from .loop import RunloomEventLoop
from .tasks import RunloomTask
from .futures import RunloomFuture
...
# Section modules, so any internal name read as runloom.aio.<name> resolves live
...
# ====================================================================
class RunloomEventLoopPolicy(asyncio.AbstractEventLoopPolicy):
    def __init__(self):
        self._loop = None
    ...
    def new_event_loop(self):
        return RunloomEventLoop()

    # Child-watcher accessors (deprecated asyncio API still asked for on Unix).
    # runloom drives subprocesses with its own per-process _wait_thread, NOT an
    # asyncio child watcher, so any watcher set here is INERT -- runloom never
...
def install():
    """Install RunloomEventLoopPolicy globally.  After this, every
    `asyncio.run(...)` / `asyncio.new_event_loop()` returns a runloom
    loop instead of the stdlib selector / proactor loop."""
    asyncio.set_event_loop_policy(RunloomEventLoopPolicy())


def run(coro, *, debug=False):
    """Drop-in for `asyncio.run`.  Creates a fresh RunloomEventLoop,
    runs `coro` to completion, returns the result.  Caller doesn't
    need to call install() first."""
    loop = RunloomEventLoop()
