...
class _SubprocessTransport(asyncio.SubprocessTransport):
    ...
    def _start_reaper(self):
        ...
        if pidfd is None:
            _threading.Thread(target=self._wait_thread,
                              name="runloom-subproc-wait", daemon=True).start()
            return
    ...
    async def _connect_pipes(self):
        ...
        # order it; runloom awaits _connect_pipes directly and it completes without
    ...
    def _process_exited(self, rc):
        ...
        # that as POLLHUP and disconnects the write pipe; runloom's stdin transport
