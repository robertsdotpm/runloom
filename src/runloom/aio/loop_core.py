"""RunloomEventLoop: lifecycle, debug, time, and exception-handler/task-
factory plumbing."""
...
class _LoopCoreMixin(object):
    def __init__(self):
        ...
        # re-raises it after runloom_c.run() returns.  None = none pending.
        ...
        # do NOT enforce thread affinity (runloom is M:N: callbacks may run
        ...
        # loop.set_task_factory() target.  None => default (build a RunloomTask);
    ...
    def _check_thread(self):
        # No-op: runloom is M:N, callbacks legitimately run on any hub
    ...
    def time(self):
        ...
        # clock when RUNLOOM_LOGICAL_CLOCK is on -- so call_at/call_later deadlines
        # line up with sched_sleep's logical deadlines and the timer schedule
        # replays deterministically (e.g. under RUNLOOM_PCT_SEED).  Identical to
        # _time.monotonic() when the logical clock is off.
        return runloom_c.loop_clock()
