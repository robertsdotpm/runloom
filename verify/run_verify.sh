...
if have spin && have cc; then
    ...
    check_spin_variant iouring_msclose BUG_CONCURRENT_CLOSE "handle refcount makes a CONCURRENT close-vs-parked-recv (shared conn) memory-safe -- no UAF"
    ...
    check_spin_must_fail iouring_msclose BUG_NO_REFCOUNT "drop the handle refcount (old code) -> the closing CQE frees while a recv is parked -> the woken recv re-locks freed memory (use-after-free)"
