 ...
 * src/pygo_core/cldeque.c, verbatim (#included below), under RC11.  This is the
 ...
 *
 * Build:  genmc -- -I<pygo_core> -DPYGO_CLDEQUE_CAP=4 chase_lev_real.c
 */
#include <pthread.h>
#include <stdatomic.h>
#include <assert.h>
 ...
 * but plat_compat.h declares (static inline, unused-here) pygo_cond_* helpers
...
static void *thief(void *u) { (void)u; rec(pygo_cldeque_steal(&d)); return 0; }
...
int main(void)
{
    ...
    pygo_cldeque_push(&d, &vals[1]);
    pygo_cldeque_push(&d, &vals[2]);

    pthread_t a, b;
    pthread_create(&a, 0, thief, 0);
    pthread_create(&b, 0, thief, 0);

    rec(pygo_cldeque_pop(&d));      /* owner pop races the two thieves */
