...
``pygo.monkey.patch()`` -- the gevent-style "run the real stdlib tests green"
...
which runs CPython's ``test_asyncio`` against ``PygoEventLoop``.

Mechanics: each CPython test method is run inside its own pygo goroutine (the
...
# PYGO_STDLIB_STACK.
STACK = int(os.environ.get("PYGO_STDLIB_STACK", str(8 << 20)))
...
# run only for boundary exploration with PYGO_STDLIB_REALTHREAD=1.  The
# cooperative primitives' GOROUTINE-driven behaviour is covered cleanly by the
# hand-adapted tests/test_*_compat.py.
REALTHREAD = os.environ.get("PYGO_STDLIB_REALTHREAD") == "1"
REALTHREAD_REASON = ("verbatim CPython tests here drive blocking primitives from "
                     ...
                     "exploratory -- set PYGO_STDLIB_REALTHREAD=1 to run")
...
class MonkeyHosted(object):
    ...
    pygo goroutine on the MAIN thread (setUp / the test / tearDown all in the
    one goroutine), driven by the C scheduler the way real ``pygo.monkey``
    ...
    def run(self, result=None):
        ...
        pygo_core.go(body, stack_size=STACK)
        pygo_core.run()
