"""pygo_core.run_ready() -- quiescence-barrier yield.
...
class TestRunReady(unittest.TestCase):
    def test_flips_wake_then_sync_boundary(self):
        ...
        def trial(use_run_ready):
            ...
            def B():
                ...
                state["removed"] = True
            def A():
                pygo_core.go(B)            # "close frame woke run_asgi"
                if use_run_ready:
                    pygo_core.run_ready()
                order.append("A")
    ...
    def test_drains_whole_cascade_not_one_pass(self):
        ...
        def C(): order.append("C"); pygo_core.go(D)
        def B(): order.append("B"); pygo_core.go(C)
        def A():
            pygo_core.go(B)
            pygo_core.run_ready()
            order.append("A")
    ...
    def test_multiple_callers_resume_fifo_no_hang(self):
        ...
        def A():
            pygo_core.go(W); pygo_core.run_ready(); order.append("A")
        def B():
            pygo_core.run_ready(); order.append("B")
        pygo_core.go(A)
        pygo_core.go(B)
        pygo_core.run()
