...
the run signature is every consumer's stream in cid order.  Same PYGO_MN_SEED
...
chans = [pygo_core.Chan() for _ in range(NCH)]
done = pygo_core.Chan()      # producers signal completion here
...
def consumer(cid):
    ...
    while openidx:
        idx, (val, ok) = pygo_core.select([("recv", chans[i]) for i in openidx])
        ci = openidx[idx]
        if not ok:
...
# initial consumers
for cid in range(4):
    pygo_core.mn_go(lambda cid=cid: consumer(cid))

# a spawner goroutine that adds two more consumers mid-run
def spawner():
    pygo_core.mn_go(lambda: consumer(100))
    pygo_core.mn_go(lambda: consumer(101))
pygo_core.mn_go(spawner)

# producers: each channel gets two producers with distinct bases
def boot():
    for ci in range(NCH):
        pygo_core.mn_go(lambda ci=ci: producer(ci, ci * 2 + 1))
        pygo_core.mn_go(lambda ci=ci: producer(ci, ci * 2 + 2))
    # join ALL producers (deterministic) before closing -- no send/close race
    for _ in range(NPROD):
        done.recv()
    for c in chans:
        c.close()
pygo_core.mn_go(boot)
