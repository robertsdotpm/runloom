...
| file | check |
| --- | --- |
| `test_stack_scrub.py` + `stack_scrub_helper.c` | S1: recycled-stack data hygiene; verifies `set_stack_scrub(True)` / `PYGO_STACK_SCRUB=1` prevents cross-goroutine stack leakage |
...
| `vg_smoke.py` + `pygo.supp` | S4: valgrind memcheck workload + by-design suppressions |
...
`PYGO_STACK_SCRUB=1`), `MADV_DONTNEED`-based so it's flat ~+8.8 us/goroutine
